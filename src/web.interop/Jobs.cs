...
public static class Jobs
{
    public static void ConfigureJobs(IApplicationBuilder app, ILogger logger)
    {
        ...
        if (BackendJobsEnabled(configuration))
        {
            ...
            RecurringJob.AddOrUpdate<secedgar.fs.SECFilingsSyncService>(
                recurringJobId: nameof(secedgar.fs.SECFilingsSyncService),
                methodCall: service => service.Execute(),
            ...
            RecurringJob.AddOrUpdate<secedgar.fs.Schedule13GProcessingService>(
                recurringJobId: nameof(secedgar.fs.Schedule13GProcessingService),
                methodCall: service => service.Execute(),
                cronExpression: "0 9 * * 1-5",  // 9:00am PT (12:00pm ET) weekdays, runs after SEC filings monitoring
                options: rjo
            );
            
            RecurringJob.AddOrUpdate<secedgar.fs.Form144ProcessingService>(
                recurringJobId: nameof(secedgar.fs.Form144ProcessingService),
                methodCall: service => service.Execute(),
                cronExpression: "30 9 * * 1-5",  // 9:30am PT (12:30pm ET) weekdays, runs after Schedule 13G job
                options: rjo
            );
            ...
            RecurringJob.AddOrUpdate<core.fs.Reports.SummaryEmailService>(
                recurringJobId: "WeeklySummaryEmail",
                methodCall: service => service.ExecuteWeekly(),
                cronExpression: Cron.Weekly(DayOfWeek.Friday, 18),
                options: rjo
            );

            RecurringJob.AddOrUpdate<core.fs.Reports.SummaryEmailService>(
                recurringJobId: "MonthlySummaryEmail",
                methodCall: service => service.ExecuteMonthly(),
                cronExpression: "0 6 1 * *",  // 6am on the 1st of every month
                options: rjo
            );
