...
namespace core.Stocks
{
    public class Buy
    {
        public class Handler : HandlerWithStorage<BuyTransaction, Unit>
        {
            public Handler(IPortfolioStorage storage) : base(storage)
            {
            }

            public override async Task<Unit> Handle(BuyTransaction cmd, CancellationToken cancellationToken)
            {
