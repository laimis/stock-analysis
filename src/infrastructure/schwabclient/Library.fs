...
type SecuritiesAccount = {
    ...
    positions: TDPosition [] option
    currentBalances: TDBalances option
}
...
type SchwabClient(blobStorage: IBlobStorage, callbackUrl: string, clientId: string, logger: ILogger<SchwabClient> option) =
    ...
    let ApiUrl = "https://api.tdameritrade.com/v1"
    let AuthUrl = "https://auth.tdameritrade.com"
    ...
    let logIfFailed (response: HttpResponseMessage) message = task {
        if not response.IsSuccessStatusCode then
            ...
            logError "TDAmeritrade client failed with {statusCode} for {message}: {content}" [|response.StatusCode; message; content|]
    }
    ...
    member this.GetAccessToken(user: UserState) = task {
        if not user.ConnectedToBrokerage then
            ...
            let storageKey = "access-token:" + user.Id.ToString()
    ...
    member private this.CallApiWithoutSerialization(user: UserState, function': string, method: HttpMethod, ?jsonData: string) = task {
        let! oauth = this.GetAccessToken(user)
    ...
    member _.ConnectCallback(code: string) = task {
        ...
        logDebug "Response from tdameritrade: {ameritraderesponse}" [|responseString|]

        return JsonSerializer.Deserialize<OAuthResponse>(responseString)
    }

    member _.GetOAuthUrl() = task {
        let encodedClientId = Uri.EscapeDataString($"{_clientId}@AMER.OAUTHAP")
        let encodedCallbackUrl = Uri.EscapeDataString(_callbackUrl)
        let url = $"{AuthUrl}/auth?response_type=code&redirect_uri={encodedCallbackUrl}&client_id={encodedClientId}"
        return url
    }
    ...
    member private this.EnterOrder(user: UserState, postData: obj) = task {
